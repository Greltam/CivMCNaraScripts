...
------------------------*/
/*
    Name: Zeal GSEx Jungle Hi Tech Chop
    Location: CivMC @ 4202, 417, 72
    Author: Greltam
    Date: 9/14/2025
    
    Description: A tower full of jungle!
    
    Directions:
        Enter sewers
        Go to entrance door @ 4201,415, 80
        Enter and take lodestone down to 4202, 417, 72
        Activate farm script.
        
    Collector: Lodestone elevator @ 4202, 417, 65
*/
/*------------------------
   0 Title End
------------------------*/
...
/*
    ...
    Pre-Start actions: 
        ...
        Fill Barrels on layers 1
            with 3 stacks of saplings
    Items Required:
        1 Diamond Efficiency 4-5 Unbreaking 3 Axes
        1 Diamond Unbreaking 3 Hoes
...
harvestDuration = 156 //minutes to run a full harvest
...
totalLayers = 12 //default: totalLayers = 8
...
greetingsText = Chat.createTextHelperFromJSON(
    util.wrapJSONStringsTogether([
        util.simpleJSONString("GSEx", "dark_aqua"),
        util.simpleJSONString(" Jungle", "yellow"),
...
function setStartingPosition(){
    ...
    //set tree
    if(startingRow % 2 === 0){
        startingTree = ((zRowEndPosition - playerZ) / treeBridgeLength) + 1
        ...
        startingTree = ((playerZ - zStartPosition) / treeBridgeLength) + 1
        startingTree = Math.floor(startingTree)
        if(startingTree <= 1){
            startingTree = 1
        }
...
//separate for even/odd rows
    if(row == 1){
        ...
        xDestination = xDestination + 0.2 //stay to the side to not hit glass
        zDestination = zRowEndPosition - 4 - ((tree-1) * 5) - 0.33
    }
...
//chop tree
    ...
    if(row % 2 == 1){
        ...
        //tippy top log
        util.simpleMove(attackKey,0, -82, floorCutTicks)
        ...
        //tippy top log
        util.simpleMove(attackKey,180, -82, floorCutTicks)
    }
...
visual.fullText("jungle","Jungle: " 
    + util.getTossedItemAmount("minecraft:jungle_log"), 0xffa500,0,16)
...
for(let i = startingLayer; i <= totalLayers; i++){
    ...
    if(!restarting){
        if((i % 4) == 1){            
            ...
            util.tossAllSpecificItems(
                ["minecraft:jungle_sapling"], -180, 0)
    ...
    for(let j = startingRow; j <= rowsPerLayer; j++){
        ...
        if(j % 2 == 1){
            ...
            tossLogs(j)
            visual.setText("jungle", "Jungle: " 
                + util.getTossedItemAmount("minecraft:jungle_log"))
